        ...
        for(int n=n_start;n<n_end;++n){
            for(int i=0;i<n2;++i){
                for(int j=0;j<n1;++j){
                    ...
                    double mxval =mx[idx];
                    double myval =my[idx];
                    double V1val = calculate_V1_rho(rhoval);
                    mx[idx] = (tau*V1val)/(tau + V1val) * (mxval/tau + nablaxphi);
                    my[idx] = (tau*V1val)/(tau + V1val) * (myval/tau + nablayphi);
                }
            }   
        }
    }
            ...
            for(int i=0;i<n2;++i){
                for(int j=0;j<n1;++j){
                    ...
                    for(int iter_newton=0;iter_newton<max_it_newton;++iter_newton){
                        ...
                        double V1dprimeval= calculate_V1_double_prime_rho(rhoval); // V1''(rho)
                        double V2val      = calculate_V2_rho(rhoval);       // V2(rho)
                        double V2primeval = calculate_V2_prime_rho(rhoval); // V2'(rho)
                        double V2dprimeval= calculate_V2_double_prime_rho(rhoval); // V1''(rho)

                        // (rho log rho - rho)' = log rho
                        double interaction_constant = 0.1;
                        double interaction_term       = log(rhoval) * interaction_constant;
                        double interaction_term_prime = 1.0/rhoval  * interaction_constant;

                        double F_rho        = - m1val*m1val/(2.0*V1val*V1val) * V1primeval  - m2val*m2val/(2.0*V2val*V2val) * V2primeval - Dtphi + (rhoval - rhotmpval) / tau + interaction_term;
                        double F_prime_rho  =   m1val*m1val/(V1val*V1val*V1val) * V1primeval * V1primeval - m1val*m1val/(2.0*V1val*V1val) * V1dprimeval
                                              + m2val*m2val/(V2val*V2val*V2val) * V2primeval * V2primeval - m2val*m2val/(2.0*V2val*V2val) * V2dprimeval
                                              + 1.0/tau + interaction_term_prime;
